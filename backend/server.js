...
// ---- INTERNAL CLEANUP ----
app.post('/api/admin/internal-clean-tests', async (req, res) => {
  try {
    const { query } = require('./db');
    // Find test users
    const { rows: testUsers } = await query(`SELECT id, email, name FROM users WHERE email LIKE '%test_owner_%' OR email LIKE '%test_part_%' OR email LIKE '%test_wait_%' OR email LIKE '%test_katilimci_%' OR email LIKE '%test_bekleyen_%' OR email = 'test_user@test.com' OR email LIKE '%live_waitlist_test%'`);
    
    // Find test events
    const { rows: testEvents } = await query(`SELECT id, title FROM posts WHERE title = 'Test Full Event' OR "isTest" = true`);

    const deletedUserIds = testUsers.map(u => u.id);
    const deletedEventIds = testEvents.map(e => e.id);

    let usersDeleted = 0;
    let eventsDeleted = 0;
    let participantsDeleted = 0;
    let waitlistsDeleted = 0;
    let notificationsDeleted = 0;

    if (deletedUserIds.length > 0) {
      const uIds = deletedUserIds.map(id => `'${id}'`).join(',');
      // Delete notifications sent TO them or BY them
      const nRes = await query(`DELETE FROM notifications WHERE "userId" IN (${uIds}) OR "relatedId" IN (${uIds})`);
      notificationsDeleted += nRes.rowCount;
      // Delete event interactions
      const pRes = await query(`DELETE FROM event_interactions WHERE "userId" IN (${uIds})`);
      participantsDeleted += pRes.rowCount;
      // Delete waitlists
      const wRes = await query(`DELETE FROM event_waitlists WHERE "userId" IN (${uIds})`);
      waitlistsDeleted += wRes.rowCount;
      
      const uRes = await query(`DELETE FROM users WHERE id IN (${uIds})`);
      usersDeleted = uRes.rowCount;
    }

    if (deletedEventIds.length > 0) {
      const eIds = deletedEventIds.map(id => `'${id}'`).join(',');
      const pRes = await query(`DELETE FROM event_interactions WHERE "eventId" IN (${eIds})`);
      participantsDeleted += pRes.rowCount;
      const wRes = await query(`DELETE FROM event_waitlists WHERE "eventId" IN (${eIds})`);
      waitlistsDeleted += wRes.rowCount;
      const nRes = await query(`DELETE FROM notifications WHERE "relatedId" IN (${eIds})`);
      notificationsDeleted += nRes.rowCount;
      
      const eRes = await query(`DELETE FROM posts WHERE id IN (${eIds})`);
      eventsDeleted = eRes.rowCount;
    }

    res.json({
      success: true,
      report: {
        testUsersFound: testUsers,
        testEventsFound: testEvents,
        usersDeleted,
        eventsDeleted,
        participantsDeleted,
        waitlistsDeleted,

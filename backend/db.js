...
const initDB = async () => {
  ...
  try {
    ...
    try {
      ...
      `);
      console.log('[DB] listing_interests table ensured.');
    } catch(e) {
      console.warn('[DB WARNING] listing_interests creation failed:', e.message);
    }
  ...
  const alters = [
    ...
    `ALTER TABLE listings ADD COLUMN IF NOT EXISTS max_stay_days_enabled BOOLEAN DEFAULT false`,
    `ALTER TABLE listings ADD COLUMN IF NOT EXISTS max_stay_days INTEGER`,
    `ALTER TABLE listings ADD COLUMN IF NOT EXISTS max_guest_count_enabled BOOLEAN DEFAULT false`,
    `ALTER TABLE listings ADD COLUMN IF NOT EXISTS max_guest_count INTEGER`,
    ...
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS "pushSettings" JSONB DEFAULT '{}'`,
    `CREATE TABLE IF NOT EXISTS listing_interests (
      id TEXT PRIMARY KEY,
      listing_id TEXT NOT NULL,
      user_id TEXT NOT NULL,

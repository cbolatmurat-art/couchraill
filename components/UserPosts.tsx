  ...
  const renderCommentItem = ({ item }: { item: any }) => {
    ...
    return (
      <View style={{ marginBottom: 16 }}>
        <Swipeable enabled={true} friction={2} rightThreshold={40} leftThreshold={40} renderRightActions={item.userId === (currentUserId || currentUser?.id || currentUser?.userId || currentUser?._id || currentUser?.email || 'unknown') ? (progress, dragX) => renderCommentRightActions(item, progress, dragX) : undefined} renderLeftActions={!(item.userId === (currentUserId || currentUser?.id || currentUser?.userId || currentUser?._id || currentUser?.email || 'unknown')) ? (progress, dragX) => renderCommentLeftActions(item, progress, dragX) : undefined}>

      ...
      <View style={{ marginBottom: 16 }}>
        <Swipeable 
          ...
          renderRightActions={(progress, dragX) => {
            const isMine = item.userId === (currentUserId || currentUser?.id || currentUser?.userId || currentUser?._id || currentUser?.email || 'unknown');
            ...
            {replies.map((reply: any) => {
              ...
              return (
                <Swipeable 
                  ...
                  rightThreshold={40} 
                  renderRightActions={(progress, dragX) => {
                    const isMine = reply.userId === (currentUserId || currentUser?.id || currentUser?.userId || currentUser?._id || currentUser?.email || 'unknown');
